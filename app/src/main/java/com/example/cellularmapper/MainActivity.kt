...
data class Tower(val lat: Double, val lon: Double, val cells: List<CellInfo>)
...
class MainActivity : AppCompatActivity(), LocationListener {
    ...
    private val eventHistory = mutableListOf<CellInfo>()
    ...
    private var lastCell: CellInfo? = null
    ...
    // EXTRACT CELL INFO
    // ================================================================
    private fun extractCellInfo(info: CellInfo): CellInfo {
    ...
    // DATABASE
    // ================================================================
    private fun saveCell(cell: CellInfo) {
        val values = ContentValues().apply {
            put("mcc", cell.mcc)
            put("mnc", cell.mnc)
            put("lac", cell.lac)
    ...
    private fun saveNeighbor(serving: CellInfo, neighbor: CellInfo) {
        // simplified
    }
    ...
    // ================================================================
    private fun detectSuspicious(cell: CellInfo, neighbors: List<CellInfo>): List<String> {
    ...
    // UI UPDATE
    // ================================================================
    private fun updateTelemetry(cell: CellInfo, neighbors: List<CellInfo>) {
    ...
    private fun updateMap(cell: CellInfo) {
    ...
    inner class HistoryAdapter(private val history: List<CellInfo>) : RecyclerView.Adapter<HistoryAdapter.ViewHolder>() {
        class ViewHolder(val textView: TextView) : RecyclerView.ViewHolder(textView)
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
            val tv = TextView(parent.context)
            tv.textSize = 9f
    ...
    // ================================================================
    // LAYOUT (included as string for reference)
    // ================================================================
    /*
    activity_main.xml should contain:
    - ScrollView with all views
    - Status, Telemetry, Stats, Map, Signal Graph, Logs, History, Buttons
    All IDs as used above.
    */
}
// ================================================================
// ================================================================
// CELLULAR MAPPER PRO — ADVANCED EXTENSION MODULE
// ================================================================
// This file extends the original MainActivity.kt above with the full
// Cellular Intelligence Platform stack (Phases 1..21).
//
// IMPORTANT:
//   * Nothing above this line was removed or modified.
//   * Everything below is ADDITIVE.
//   * All advanced features live in their own classes/objects and can
//     be wired into the existing UI gradually (call sites are documented
//     at the bottom of this file under "INTEGRATION NOTES").
//   * Designed to compile as a single Kotlin source unit alongside the
//     original code (same package: com.example.cellularmapper).
//
...
// ================================================================
// Full schema: Tower, Measurement, Sector, SurveySession
// ================================================================
...
// ================================================================
// Pipeline: RSRP→distance, Weighted Least Squares, Weighted Centroid,
//           Confidence Scoring, Kalman Filter, Outlier Removal.
// ================================================================

data class EstimatedPosition(val lat: Double, val lon: Double, val confidence: Double, val radius: Double)

object PathLoss {
    /** Hata-like log-distance model. Returns distance (m) from RSRP dBm. */
...
object TowerEstimator {
    ...
    /** Simplified 2D weighted least squares around an initial centroid. */
...
// ================================================================
// PHASE 5 — COVERAGE MAPPING (Heatmap / Interpolation / Dead Zones)
// ================================================================
...
object CoverageMapper {
    ...
    /** IDW interpolation onto a grid. Cell side ~50m. */
...
object BandAnalyzer {
    // Abbreviated LTE band table (production builds should ship full table).
...
// ================================================================
// PHASE 12 — OFFLINE GEOLOCATION (Self-Improving Index)
// ================================================================
...
// ================================================================
// PHASE 13 — SMART CLUSTERING (DBSCAN, K-Means, Hierarchical)
// ================================================================

object Clustering {
    /** DBSCAN by Haversine distance (meters). */
    fun dbscan(points: List<Pair<Double, Double>>, epsMeters: Double, minPts: Int): List<Int> {
        val n = points.size
        val labels = IntArray(n) { -1 }  // -1 unvisited, -2 noise
        var cluster = 0
        for (i in 0 until n) {
...
object SignalPredictor {
    /** Predict RSRP at (lat,lon) using inverse-distance-weighted historical samples. */
...
class OfflineTileProvider(private val mbtilesPath: String) {
    ...
    /** Returns raw PNG bytes for a tile from an MBTiles archive (callers wire to osmdroid). */
    fun tile(z: Int, x: Int, y: Int): ByteArray? {
        if (!isAvailable()) return null
        // Production: open with SQLiteDatabase.openDatabase and SELECT tile_data
...
/**
 * Foreground service template for long-running scans. Register in
 * AndroidManifest.xml as a foreground service of type "location".
 */
...
// ================================================================
// PHASE 21 — UI HELPERS (Dark Cyber Theme tokens + view-model)
// ================================================================
...
// ================================================================
// Single entry point that consumes a raw CellInfo (from the original
// code at the top of this file) and feeds the full platform.
// ================================================================
...
class CellularPipeline(ctx: Context) {
    ...
    /** Ingest a raw CellInfo plus location into the platform. */
    suspend fun ingest(cell: CellInfo) = withContext(Dispatchers.IO) {
    ...
    private suspend fun upsertTower(c: CellInfo, band: String?): TowerEntity {
...
// ================================================================
// 1) In MainActivity (above), after constructing CellInfo from telephony
//    callbacks, call:
...
//   22 new professional engines layered on top of CellularPipeline
// ================================================================
// ================================================================
//
//   Naming convention: all new symbols use the `Adv` prefix to avoid
//   any collision with existing classes in this file. Existing
//   classes, functions, DAOs, entities and UI are preserved intact.
//
//   Wire-up (one line in MainActivity):
//       val advanced = AdvancedSuite(this, pipeline)
//       lifecycleScope.launch { advanced.ingest(cellInfo, gps) }
//
// ================================================================
...
//    Weighted Centroid → Least Squares → TA Constraints → Confidence
// ----------------------------------------------------------------
object AdvTowerGeolocation {
    private fun rsrpWeight(rsrp: Double): Double {
        // stronger signal => higher weight (closer to tower assumed)
    ...
    private fun taDistance(ta: Int): Double = ta.coerceAtLeast(0) * 78.12 // LTE meters

    // Gauss–Newton least squares minimizing  Σ w_i (||p_i - x|| - r_i)^2
    fun leastSquaresRefine(
        ...
        for (it in 0 until iterations) {
            ...
            for (s in samples) {
                ...
                val r = if (s.ta > 0) taDistance(s.ta) else d  // when TA absent: zero residual
...
object AdvMultilateration {
    ...
    // Closed-form linear intersection for k circles (anchors) with distances.
...
// ----------------------------------------------------------------
// 3) Kalman Filter (1D per channel + 2D for GPS)
// ----------------------------------------------------------------
...
// ----------------------------------------------------------------
// 4) Particle Filter for moving-vehicle tower tracking
// ----------------------------------------------------------------
...
// ----------------------------------------------------------------
// 6) Circular Statistics Engine (Von Mises)
// ----------------------------------------------------------------
object AdvCircularStats {
    ...
    // Concentration parameter approximation
...
object AdvPropagation {
    ...
    // Okumura–Hata (urban). hb: BS height, hm: MS height (meters), d in km.
    ...
    // Estimate distance from observed RSRP using a path-loss inversion.
...
object AdvCoverageEngine {
    fun predict(samples: List<AdvSample>, cellM: Double = 50.0): AdvCoverageGrid {
        ...
        val mPerLat = 111_320.0
        val mPerLon = 111_320.0 * cos(Math.toRadians(ref.lat))
        val acc = HashMap<Pair<Int, Int>, Pair<Double, Double>>() // sumW, sumWV
        for (s in samples) {
...
// ----------------------------------------------------------------
// 9) Heatmap Engine (IDW + bilinear, tile-cached)
// ----------------------------------------------------------------
...
// ----------------------------------------------------------------
// 13) Fake Cell Detection Engine (weighted risk + optional ML)
// ----------------------------------------------------------------
...
// ----------------------------------------------------------------
// 14) Band Detection Engine (extended LTE + NR ranges)
// ----------------------------------------------------------------
object AdvBandDetector {
    ...
    // Reference frequencies use NR-ARFCN ranges in FR1
...
// ----------------------------------------------------------------
// 20) Offline Intelligence Database (in-memory cache; persisted via Room)
// ----------------------------------------------------------------
...
// ================================================================
class AdvancedSuite(
    private val ctxOwner: Any?,           // pass Activity / Context — kept generic
    private val pipeline: Any?            // existing CellularPipeline (untouched)
) {
